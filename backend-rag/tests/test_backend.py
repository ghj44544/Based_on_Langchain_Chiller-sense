...
def test_diagnosis_upload_returns_python_prediction():
    df = sample_dataframe()
    buffer = io.StringIO()
    ...
    if response.status_code == 400:
        assert any(
            ...
            for phrase in ["模型文件不存在", "MATLAB Engine", "模型期望"]
        )
        return
    ...
    payload = response.json()
    assert payload["code"] == 200
    assert payload["data"]["model_result"]["total_rows"] == 2
    assert "dominant_fault_name" in payload["data"]["model_result"]
